...
// This is a collection of commonly used routines that I've been using in my code
// I take no responsibility for how you use this code. You can copy it and use it
// however you wish, but please place a notice giving me credit.
...
namespace prettyPrint {
	...
	std::ostream &operator<<(std::ostream &stream, const printWrapper<std::chrono::duration<Rep, Period>> &&duration) {
		return stream << std::setprecision(2) << std::fixed << (float)std::chrono::duration_cast<std::chrono::microseconds>(duration.data).count() / 1000 << "ms";
	}
